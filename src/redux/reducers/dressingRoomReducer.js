...
const dressingRoomReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case TRY_ON_CLOTHES: {
            let dataTryOnClothesUpdate = [...state.dataTryOnClothes];
            let indexDressing = dataTryOnClothesUpdate.findIndex(item => item.type === action.dressing.type);
            if (indexDressing !== -1) {
                dataTryOnClothesUpdate[indexDressing] = action.dressing;
            } else {
                dataTryOnClothesUpdate.push(action.dressing);
            }
            state.dataTryOnClothes = [...dataTryOnClothesUpdate];
            return { ...state };
        }
